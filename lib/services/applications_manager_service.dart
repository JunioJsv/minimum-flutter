...
class ApplicationsManagerService with WidgetsBindingObserver {
  ...
  late final eventsStream = eventsChannel.receiveBroadcastStream().map((event) {
  ...
  Future<void> launchApplication(String component) async {
    await channel.invokeMethod(kLaunchApplication, {
  ...
  Future<Uint8List> getIconFromIconPack(
    ...
    final bytes = await _icons.get([package, drawable].join('/'), () async {
      final bytes = await channel.invokeMethod<Uint8List>(
        kGetIconFromIconPack,
        {'package_name': package, 'drawable_name': drawable, 'size': size},
      );
  ...
  Future<Uint8List> getApplicationIcon([
    ...
    final bytes = await _icons.get(component ?? 'default', () async {
      final bytes = await channel.invokeMethod<Uint8List>(kGetApplicationIcon, {
  ...
  Future<Uint8List> getPackageIcon([String? package, int size = 96]) async {
    final bytes = await _icons.get(package ?? 'default', () async {
      final bytes = await channel.invokeMethod<Uint8List>(kGetPackageIcon, {
  ...
  Future<bool> isAlreadyCurrentLauncher() async {
    final result = await channel.invokeMethod<bool>(kIsAlreadyCurrentLauncher);
  ...
  Future<void> openApplicationDetails(String component) async {
    await channel.invokeMethod(kOpenApplicationDetails, {
      'component_name': component,
    });
  }

  Future<void> uninstallPackage(String package) async {
    await channel.invokeMethod(kUninstallPackage, {'package_name': package});
  }

  Future<ApplicationBase> getApplication(String component) async {
    final json = await channel.invokeMethod(kGetApplication, {
      'component_name': component,
    });
  ...
  Future<IList<ApplicationBase>> getPackageApplications(String package) async {
    final json = await channel.invokeListMethod<Map>(kGetPackageApplications, {
  ...
  Future<bool> setIconPack(String? package) async {
    final isIconPackApplied = await channel
        .invokeMethod<bool>(kSetIconPack, {
          if (package != null) 'package_name': package,
        })
  ...
  Future<bool> isPackageEnabled(String package) async {
    final isApplicationEnabled = await channel
        .invokeMethod<bool>(kIsPackageEnabled, {'package_name': package})
        .then((value) => value ?? false);
  ...
  Future<Map<String, String>> getIconPackDrawables(String package) async {
    final drawables = await channel.invokeMethod<Map>(kGetIconPackDrawables, {
      'package_name': package,
    });
